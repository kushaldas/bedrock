...
class TestNewsletter(TestCase):
    ...
    def test_language_selected(self):
        ...
        doc = pq(resp.content)
        eq_(doc('#id_language option[selected="selected"]').val(), 'fr')

        with self.activate('pt-BR'):
            resp = self.client.get(reverse(self.view_name))

        doc = pq(resp.content)
        eq_(doc('#id_language option[selected="selected"]').val(), 'pt-BR')

        with self.activate('ak'):
            resp = self.client.get(reverse(self.view_name))

        doc = pq(resp.content)
        eq_(doc('#id_language option[selected="selected"]').val(), 'en-US')
